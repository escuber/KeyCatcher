...
static const uint16_t KC_MAX_ACCUM = 4096;       // max assembled msg bytes, tune as needed
...
void parseAndSaveConfig(String setupMsg)
{
  ...
  Serial.println(config.ap_mode ? "Enabled" : "Disabled");
  Serial.print("InputSource=");
  Serial.println(config.input_source);
  Serial.print(" output=");
  Serial.println(config.output_source);
  Serial.print(" blink flag=");
  Serial.println(config.blink_flag);
...
class KC_OnWrite : public BLECharacteristicCallbacks
{
  void onWrite(BLECharacteristic *pChr) override
  {
    ...
    // Normal text: ensure <<END>> optional
    if (!s.endsWith("<<END>>"))
      s += "<<END>>";
    processIncoming(s);
  }
...
void loadConfig()
{
  ...
  config.ssid = preferences.getString("ssid", "NotSet");
  config.password = preferences.getString("password", "4c4c4c4c");
...
void loop()
{
  ...
  while (!gBleRxQ.empty())
  {
    ...
    gBleRxQ.pop_front();
    ///Serial.printf("[BLE dequeue] bytes=%u first='%c'\n",
      //            (unsigned)v.size(), v.empty() ? '?' : v[0]);
    interrupts();
